...
function PetChatbot() {
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(
        "pawguide-knowledge-cards",
        JSON.stringify(knowledgeCards),
      );
      localStorage.setItem(
        "pawguide-pet-profiles",
        JSON.stringify(petProfiles),
      );
      localStorage.setItem(
        "pawguide-chat-histories",
  ...
  const lastUserMessage =
    messages.length > 0
      ? messages.filter((m) => m.role === "user").pop()?.content || null
      : null;
  ...
  const splitIntoChunks = (text: string, chunkSize: number): string[] => {
    ...
    while (currentPosition < text.length) {
      ...
      if (patternMatch) {
        ...
        chunks.push(
          text.slice(currentPosition, currentPosition + patternLength),
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        "Failed to connect to the pet assistant. Please try again later.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const getThemeColors = () => {
    const baseColors = {
      ...
      emerald: {
        ...
        bg:
          theme === "dark" ? "bg-emerald-900 bg-opacity-70" : "bg-emerald-100",
        border: theme === "dark" ? "border-emerald-800" : "border-emerald-200",
        hover:
          theme === "dark" ? "hover:bg-emerald-800" : "hover:bg-emerald-200",
        button:
          theme === "dark"
      ...
      <Dialog open={showShortcutsDialog} onOpenChange={setShowShortcutsDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Use these keyboard shortcuts to navigate PawGuide more
              efficiently.
            </DialogDescription>
          </DialogHeader>
      ...
      <div className="flex flex-col flex-1">
        ...
        >
          <div className="container flex items-center h-16 px-4 mx-auto max-w-4xl">
            ...
            <h1 className={cn("text-xl font-bold", colors.primary)}>
              PawGuide Assistant
            </h1>
            ...
            {activePet && (
              <Badge
                variant="outline"
                className={cn("ml-2", colors.border, colors.primary)}
              >
              ...
              {messages.length === 0 ? (
                ...
                >
                  <motion.div
                    ...
                    className={
                      theme === "dark"
                        ? "bg-gray-800 p-6 rounded-full"
                        : "bg-gray-100 p-6 rounded-full"
                    }
                  >
                    <PetIcon className={`w-16 h-16 ${petColor}`} />
                  </motion.div>
                  <h2 className={cn("text-2xl font-semibold", colors.primary)}>
                    Welcome to PawGuide!
                  </h2>
                  <p
                    className={
                  ...
                  >
                    Ask me anything about pet care, behavior, training,
                      ...
                      <div className="flex flex-wrap gap-2 justify-center">
                        {petProfiles.map((pet) => (
                          <Badge
                            key={pet.id}
                            variant={
                              activePet?.id === pet.id ? "default" : "outline"
                            }
                            className={cn(
                            ...
                            <ReactMarkdown
                              ...
                              components={{
                                ...
                                blockquote: ({ node, ...props }) => (
                                  <blockquote
                                    {...props}
                                    className={`border-l-4 pl-4 italic my-2 ${colors.border}`}
                                  />
                                ),
                                hr: ({ node, ...props }) => (
                                  <hr
                              ...
                              <Dialog>
                                ...
                                <DialogContent>
                                  <DialogHeader>
                                    ...
                                    <DialogDescription>
                                      Create a knowledge card from this
              ...
              {[
                ...
                >
                  <div className="mb-3">
                    ...
                    >
                      <AvatarFallback
                        className={cn("text-xl font-bold", colors.primary)}
                      >
                        {member.avatar}
                      </AvatarFallback>
                    </Avatar>
